...
服务器在原因码为 0x9C（临时使用另一台服务器）或 0x9D（服务器迁移）的 CONNACK 或 DISCONNECT 包中使用服务引用，具体用法参考 [4.13](#4-13-错误处理)。
...
| 156 | 0x9C | 临时使用另一台服务器 | 服务器 | 客户端需要临时使用其他服务器。 |
...
服务器发送原因码为 0x9C（临时使用另一台服务器）或 0x9D（服务器迁移）的 DISCONNECT 包时包括服务引用字段，参考 [4.13](#4-13-错误处理)。
